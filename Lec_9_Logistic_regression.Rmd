...
### 3.1 Numeric example
I was interesting, whether languages with ejective sounds have in average more consonants. So I collected data from phonological database LAPSyD: http://goo.gl/0btfKa.
...
log(10/17)
```
What does this model says? This model says that if we have no any predictors and take some language it have $\frac{0.5306283}{(1+e^{-0.5306283})} = 0.3340993$ probability to have an ejectives.
...
summary(fit2)
```
What does this model says? This model says:
...
So we actually can create a plot with a confidense intervals.
```{r}
